...
void printPreview(){
	...
	{
		for (int x = 0; x < 4; x++){
			if (1 <= Blocks[Block][Rotation][x][y] && Blocks[Block-1][Rotation][x][y] <= 7)
			{
				DrawTetrisBlock(colors[Blocks[Block][Rotation][x][y]], (point2d_t){x+X_offset, y});
			}
			else
			{
...
void movePiece(char input, Playfield *field)
{
    ...
    { // Move down
        ...
        {                                                              // Hit bottom or immovable block
            ...
            xQueueReceive(xRotation_Queue, &player_piece_rotation, portMAX_DELAY);
    ...
    itemsInQueue = uxQueueMessagesWaiting(xRotation_Queue);
    if (itemsInQueue <= 1)
    {
    	rotation = rand() % 4;
        xQueueSend(xRotation_Queue, &rotation, 0UL);
    }
